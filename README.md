...
可以在 Settings 中调整任务栏字体大小，改动会立即应用到任务栏小显示块。
任务栏内的小显示块只显示百分数，上方是 `5h84%`，下方是 `周97%`；它会自动放在任务栏按钮区域和系统托盘之间，如果空间不足会自动隐藏，避免遮挡其他程序块。
主窗口的 `Token Meter` 页会扫描本机支持的 AI 编程助手会话日志，显示总 tokens、估算成本、客户端汇总和模型明细。成本使用公开 LiteLLM/OpenRouter 价格数据估算，未知模型不会按 0 计算。
...
- Supports taskbar font size adjustment from Settings.
